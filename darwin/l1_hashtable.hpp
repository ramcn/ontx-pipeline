...
class Hashtable{
  ...
  public:
    ...
    unsigned long long int get_frequency_address(uint64_t hash);
    uint32_t get_offset(uint64_t hash);
    unsigned long long int get_offset_address(uint64_t hash);
    ...
    unsigned long long int get_location_address(uint32_t index);
