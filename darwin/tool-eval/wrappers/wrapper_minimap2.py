...
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/minimap2/';
BIN = 'minimap2';
MAPPER_NAME = 'MINIMAP2';
...
def run(reads_file, reference_file, machine_name, output_path, output_suffix=''):
	...
	sys.stderr.write('[%s wrapper] Running %s...\n' % (MAPPER_NAME, MAPPER_NAME));
        command = '%s %s/%s %s -ax map-ont  %s  %s > %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, parameters, reference_file, reads_file, sam_file);
