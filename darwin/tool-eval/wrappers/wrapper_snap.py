...
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/snap/';
BIN = 'snap-aligner';
MAPPER_NAME = 'SNAP';
...
def run(reads_file, index_dir, machine_name, output_path, output_suffix=''):
	parameters = '';
	...
	sys.stderr.write('[%s wrapper] Running %s...\n' % (MAPPER_NAME, MAPPER_NAME));
        command = '%s %s/%s single %s %s -o %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, index_dir, reads_file, sam_file);
...
def verbose_usage_and_exit():
	...
	sys.stderr.write('\t%s mode [<reads_file> <index_dir> <machine_name> <output_path> [<output_suffix>]]\n' % sys.argv[0]);
	sys.stderr.write('\n');
...
if __name__ == "__main__":
	...
	if (sys.argv[1] == 'install'):
		...
		run(reads_file, index_dir, machine_name, output_path, output_suffix);
