...
#ALIGNER_PATH = SCRIPT_PATH + '/../aligners/darwin_v1/software';
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/darwin-algo-dev';
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/darwin-algo-v3';
#ALIGNER_PATH = SCRIPT_PATH + '/../aligners/darwin_v2/software';
BIN = 'darwin_ref_guided';
#BIN = 'Darwin';
MAPPER_NAME = 'DARWIN';
...
def run(reads_file, reference_file, machine_name, output_path, output_suffix=''):
	...
	sys.stderr.write('[%s wrapper] Running %s...\n' % (MAPPER_NAME, MAPPER_NAME));
        #BIN = sys.argv[1]
        print sys.argv
	command = '%s/%s %s %s %s %s %s %s %s > %s' % (ALIGNER_PATH, BIN, reference_file, reads_fasta, sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], maf_file);
	#command = '%s %s/%s %s %s 0 > %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, reference_file, reads_fasta, maf_file);
	#command = '%s %s/%s %s %s %s %s %s %s %s > %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, reference_file, reads_fasta, sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], maf_file);
	#command = '%s %s/%s %s %s 0 > %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, reference_file, reads_fasta, maf_file);
	#command = '%s %s/%s %s %s  > %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, reference_file, reads_fasta, maf_file);
	...
	fp.close();
	command = '%s/../scripts/maf-convert.py sam %s >> %s' % (ALIGNER_PATH, maf_file, sam_file);
	sys.stderr.write('[%s wrapper] %s\n' % (MAPPER_NAME, command));
	subprocess.call(command, shell=True);
	sys.stderr.write('\n\n');
